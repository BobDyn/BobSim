...
BobVis: a PyQt6 + PyVista desktop viewer (`viewer.py`) and a headless video exporter (`run_visual.py`), both driving the same scene model (`scene.py`), plus the visual templates that map signals to geometry. Two numpy-only modules hold the arithmetic, so it tests in CI without Qt or VTK: `navigation.py` (mouse and touchpad camera control) and `tire_state.py` (LLTD, and friction-circle grip use from the `.tir` MF5.2 terms). Replays hardpoint geometry captured from StandardSim runs by `from_results.py`; `_5_App` serves the same templates. Runs on the host rather than in the container — a containerised window leaves its 3D viewport black under software GL — and its Qt/VTK dependencies install separately via `make visual-deps`, so the simulation stack never carries them. See [`../_1_VisualSim/README.md`](../_1_VisualSim/README.md). (Live model visualization during development still happens in OMEdit.)
