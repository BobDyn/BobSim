...
try:
    ...
    raise ImportError(
        "RampSteerEval requires scipy.interpolate.UnivariateSpline. "
        "Install scipy to run the ramp-steer analysis."
    ) from exc
...
RampSteerEval_VARIABLE_FILTER = "|".join(RampSteerEval_SIGNALS)
...
class RampSteerEvalSim:
    def __init__(self, config: dict[str, Any]):
        self.config = config

        sim_cfg = config.setdefault("simulation", {})
        sim_cfg.setdefault("variable_filter", RampSteerEval_VARIABLE_FILTER)
        backend = sim_cfg.get("backend", "modelica")

        if backend != "modelica":
            raise ValueError(
                "RampSteerEvalSim currently expects simulation.backend='modelica'. "
                f"Got {backend!r}."
            )
        ...
        *arrays: np.ndarray,
        ...
        for arr in arrays:
            ...
            if arr.shape != time.shape:
                raise ValueError(
                    "RampSteerEval result series must share a common sample axis."
                )
        ...
        def _velocity_key(result: dict[str, Any]) -> float:
            for key in ("_testVel", "targetVel"):
                ...
                try:
                    ...
                    continue
            raise KeyError(
                "RampSteerEval result is missing velocity metadata "
    ...
    def write_metrics_csv(self, metrics: list[dict[str, Any]]) -> Path:
        ...
        report_path = Path(
            report_cfg.get(
                "output_path",
                "_3_StandardSim/results/ramp_steer_eval_report.pdf",
            )
        )
    ...
    def summarize(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        if not results:
            raise ValueError("No RampSteerEval results to summarize.")
        ...
        if not successful_results:
            ...
            raise RuntimeError(
                "All RampSteerEval cases failed; nothing to summarize. "
        ...
        def _summarize_raw_results(raw_subset: list[dict[str, Any]]) -> dict[str, Any]:
            ...
            if ay_param.size == 0:
                raise ValueError(
                    "RampSteerEval produced no usable Ay samples after the configured cutoff."
                )
        ...
        metrics: list[dict[str, Any]] = [
            {
                ...
                "value": len(results),
                "units": "count",
                "description": "Total RampSteerEval cases requested",
            },
            {
                "standard": "RampSteerEval",
                ...
                "units": "count",
                "description": "Number of successful RampSteerEval cases used in the summary",
            },
            {
                "standard": "RampSteerEval",
                ...
                "units": "count",
                "description": "Number of failed RampSteerEval cases skipped by the summary",
            },
            {
                "standard": "RampSteerEval",
                ...
                "value": sweep_max_ay,
                "units": "m/s^2",
                "description": "Nominal positive lateral-acceleration target used for the RampSteerEval sweep",
            },
            {
                "standard": "RampSteerEval",
...
def main(path: str | Path | None = None) -> dict[str, Any]:
    ...
    result = RampSteerEvalSim(config).run()
