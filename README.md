...
vehicle dynamics studies, design-of-experiments runs, post-processing, and
report generation.
...
- `_0_Utils/`: shared Python utilities, plotting/reporting helpers, and the
  BobLib submodule.
- `_1_VisualSim/`: visual simulation tools.
...
- `_3_StandardSim/`: standard vehicle studies: SteadyStateEval, TransientEval,
  and FourPostEval.
- `_4_OptSim/`: DOE and sensitivity workflows.
...
Build and run the standard studies:

```bash
make sim-steady-state
make sim-transient
make sim-four-post
```
