from _0_Utils.kin_py.suspension_model.suspension_elements._5_elements.quarter_car import QuarterCar
from _0_Utils.kin_py.suspension_model.suspension_elements._2_elements.stabar import Stabar
...
class Suspension:
    ...
    def __post_init__(self) -> None:
        ...
        self.state: dict[str, float] = {"veh_CG_x": self.veh_CG[0],
                                        ...
                                        "FL_bump_spring_MR": self.FL_bump_spring_MR,
                                        "FR_bump_spring_MR": self.FR_bump_spring_MR,
                                        "RL_bump_spring_MR": self.RL_bump_spring_MR,
                                        "RR_bump_spring_MR": self.RR_bump_spring_MR,
                                        "Fr_roll_stabar_MR": self.Fr_stabar_MR[0],
                                        "Fr_bump_stabar_MR": self.Fr_stabar_MR[1],
                                        "Rr_roll_stabar_MR": self.Rr_stabar_MR[0],
                                        "Rr_bump_stabar_MR": self.Rr_stabar_MR[1],
                                        "Fr_track": self.Fr_track,
                                        "Rr_track": self.Rr_track,
                                        "Avg_wheelbase": self.avg_wheelbase,
                                        "FL_wheelrate": self.FL_wheelrate,
                                        "FR_wheelrate": self.FR_wheelrate,
                                        "RL_wheelrate": self.RL_wheelrate,
                                        "RR_wheelrate": self.RR_wheelrate,
                                        "Fr_Kr": self.Fr_Kr,
                                        "Rr_Kr": self.Rr_Kr,
                                        "Avg_Kp": self.avg_Kp,}
        ...
        self.sprung_mass = self.sus_data.sprung_mass
        self.Fr_unsprung_mass = self.sus_data.Fr_unsprung_mass
        self.Rr_unsprung_mass = self.sus_data.Rr_unsprung_mass
    ...
    def _mech_trail_calculation(self, CP_1: Node, CP_2: Node, CP_3: Node, quarter_car: QuarterCar):
        ...
        mech_trail_mag = abs(cp_proj[0] - inter_proj[0])
    ...
    def _update_state(self) -> None:
        ...
        self.state["FL_bump_spring_MR"] = self.FL_bump_spring_MR
        self.state["FR_bump_spring_MR"] = self.FR_bump_spring_MR
        self.state["RL_bump_spring_MR"] = self.RL_bump_spring_MR
        self.state["RR_bump_spring_MR"] = self.RR_bump_spring_MR
        if self.Fr_stabar_MR:
            self.state["Fr_roll_stabar_MR"] = self.Fr_stabar_MR[0]
            self.state["Fr_bump_stabar_MR"] = self.Fr_stabar_MR[1]
        if self.Rr_stabar_MR:
            self.state["Rr_roll_stabar_MR"] = self.Rr_stabar_MR[0]
            self.state["Rr_bump_stabar_MR"] = self.Rr_stabar_MR[1]
        self.state["Fr_track"] = self.Fr_track
        self.state["Rr_track"] = self.Rr_track
        self.state["Avg_wheelbase"] = self.avg_wheelbase
        self.state["FL_wheelrate"] = self.FL_wheelrate
        self.state["FR_wheelrate"] = self.FR_wheelrate
        self.state["RL_wheelrate"] = self.RL_wheelrate
        self.state["RR_wheelrate"] = self.RR_wheelrate
        self.state["Fr_Kr"] = self.Fr_Kr
        self.state["Rr_Kr"] = self.Rr_Kr
        self.state["Avg_Kp"] = self.avg_Kp
    ...
    def FL_N_lines(self) -> Tuple[Link, Link]:
        ...
        FL_FVIC = np.array(self.FL_FVIC)
        FL_SVIC = np.array(self.FL_SVIC)

        FVIC_N_line_start = (FL_FVIC - FL_cp_pos) / np.linalg.norm((FL_FVIC - FL_cp_pos)) * FL_tire.outer_diameter / 2 + FL_cp_pos
        FVIC_N_line_end = -1 * (FL_FVIC - FL_cp_pos) / np.linalg.norm((FL_FVIC - FL_cp_pos)) * FL_tire.outer_diameter / 2 + FL_cp_pos
        
        SVIC_N_line_start = (FL_SVIC - FL_cp_pos) / np.linalg.norm((FL_SVIC - FL_cp_pos)) * FL_tire.outer_diameter / 2 + FL_cp_pos
        SVIC_N_line_end = -1 * (FL_SVIC - FL_cp_pos) / np.linalg.norm((FL_SVIC - FL_cp_pos)) * FL_tire.outer_diameter / 2 + FL_cp_pos

        FVIC_N_line_link = Link(inboard_node=Node(position=FVIC_N_line_start), outboard_node=Node(position=FVIC_N_line_end))
        SVIC_N_line_link = Link(inboard_node=Node(position=SVIC_N_line_start), outboard_node=Node(position=SVIC_N_line_end))

        return (FVIC_N_line_link, SVIC_N_line_link)
    ...
    def FR_N_lines(self) -> Tuple[Link, Link]:
        ...
        FR_FVIC = np.array(self.FR_FVIC)
        FR_SVIC = np.array(self.FR_SVIC)

        FVIC_N_line_start = (FR_FVIC - FR_cp_pos) / np.linalg.norm((FR_FVIC - FR_cp_pos)) * FR_tire.outer_diameter / 2 + FR_cp_pos
        FVIC_N_line_end = -1 * (FR_FVIC - FR_cp_pos) / np.linalg.norm((FR_FVIC - FR_cp_pos)) * FR_tire.outer_diameter / 2 + FR_cp_pos
        
        SVIC_N_line_start = (FR_SVIC - FR_cp_pos) / np.linalg.norm((FR_SVIC - FR_cp_pos)) * FR_tire.outer_diameter / 2 + FR_cp_pos
        SVIC_N_line_end = -1 * (FR_SVIC - FR_cp_pos) / np.linalg.norm((FR_SVIC - FR_cp_pos)) * FR_tire.outer_diameter / 2 + FR_cp_pos
    ...
    def RL_N_lines(self) -> Tuple[Link, Link]:
        ...
        RL_FVIC = np.array(self.RL_FVIC)
        RL_SVIC = np.array(self.RL_SVIC)

        FVIC_N_line_start = (RL_FVIC - RL_cp_pos) / np.linalg.norm((RL_FVIC - RL_cp_pos)) * RL_tire.outer_diameter / 2 + RL_cp_pos
        FVIC_N_line_end = -1 * (RL_FVIC - RL_cp_pos) / np.linalg.norm((RL_FVIC - RL_cp_pos)) * RL_tire.outer_diameter / 2 + RL_cp_pos
        
        SVIC_N_line_start = (RL_SVIC - RL_cp_pos) / np.linalg.norm((RL_SVIC - RL_cp_pos)) * RL_tire.outer_diameter / 2 + RL_cp_pos
        SVIC_N_line_end = -1 * (RL_SVIC - RL_cp_pos) / np.linalg.norm((RL_SVIC - RL_cp_pos)) * RL_tire.outer_diameter / 2 + RL_cp_pos
    ...
    def RR_N_lines(self) -> Tuple[Link, Link]:
        ...
        RR_FVIC = np.array(self.RR_FVIC)
        RR_SVIC = np.array(self.RR_SVIC)

        FVIC_N_line_start = (RR_FVIC - RR_cp_pos) / np.linalg.norm((RR_FVIC - RR_cp_pos)) * RR_tire.outer_diameter / 2 + RR_cp_pos
        FVIC_N_line_end = -1 * (RR_FVIC - RR_cp_pos) / np.linalg.norm((RR_FVIC - RR_cp_pos)) * RR_tire.outer_diameter / 2 + RR_cp_pos
        
        SVIC_N_line_start = (RR_SVIC - RR_cp_pos) / np.linalg.norm((RR_SVIC - RR_cp_pos)) * RR_tire.outer_diameter / 2 + RR_cp_pos
        SVIC_N_line_end = -1 * (RR_SVIC - RR_cp_pos) / np.linalg.norm((RR_SVIC - RR_cp_pos)) * RR_tire.outer_diameter / 2 + RR_cp_pos
    ...
    @property
    def Fr_track(self) -> float:
        return abs((np.array(self.FL_cp) - np.array(self.FR_cp))[1])

    @property
    def Rr_track(self) -> float:
        return abs((np.array(self.RL_cp) - np.array(self.RR_cp))[1])
    
    @property
    def avg_wheelbase(self) -> float:
        return abs((np.array(self.FL_cp) - np.array(self.RL_cp))[0])
    
    @property
    def FL_wheelrate(self) -> float:
        return self.sus_data.FL_quarter_car.push_pull_rod.spring.compliance / self.FL_bump_spring_MR**2

    @property
    def FR_wheelrate(self) -> float:
        return self.sus_data.FR_quarter_car.push_pull_rod.spring.compliance / self.FR_bump_spring_MR**2
    
    @property
    def RL_wheelrate(self) -> float:
        return self.sus_data.RL_quarter_car.push_pull_rod.spring.compliance / self.RL_bump_spring_MR**2
    
    @property
    def RR_wheelrate(self) -> float:
        return self.sus_data.RR_quarter_car.push_pull_rod.spring.compliance / self.RR_bump_spring_MR**2
    
    @property
    def Fr_Kr(self) -> float:
        return 1 / 4 * (self.FL_wheelrate + self.FR_wheelrate) * self.Fr_track**2
    
    @property
    def Rr_Kr(self) -> float:
        return 1 / 4 * (self.RL_wheelrate + self.RR_wheelrate) * self.Rr_track**2
    
    @property
    def avg_Kp(self) -> float:
        return 1 / 4 * (self.FL_wheelrate + self.RL_wheelrate) * self.avg_wheelbase**2